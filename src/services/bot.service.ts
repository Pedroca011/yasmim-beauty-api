...
class BotService {
  async handleMessage(inputs: {
    ...
    const normalizedMessage = message.toLowerCase().trim().replace(/\s+/g, ""); // Remove espaços internos para permitir "1, 2"
    console.log("[BOT SERVICE] Mensagem normalizada:", normalizedMessage);

    const phone = remoteJid.split("@")[0];
    // const adminPhone = "553898465614";
    ...
    if (phone === adminPhone && normalizedMessage === "admin") {
      ...
      if (normalizedMessage.startsWith("cancelar")) {
        ...
        const appointments = await appointmentRepository.findByClientId(
          user.id,
        );
      ...
      switch (normalizedMessage) {
        ...
        case "start":
          ...
          const buttons = products.slice(0, 3).map((p, i) => ({
            title: `${i + 1}. ${p.name} - R$ ${p.currentPrice.toFixed(2)}`,
            displayText: `${i + 1}. ${p.name} - R$ ${p.currentPrice.toFixed(2)}`,
            id: `btn_${i + 1}`,
          }));

          console.log("[BOT SERVICE] Buttons gerados:", buttons);
          ...
          return {
            ...
            description: `Olá ${pushName}, seja bem-vindo(a) ao Auto atendimento do Studio Beauty. \n\n Selecione um ou mais serviços abaixo \n Ex: 1,2,3 ou 1`,
            footer: "Studio Bot",
            buttons,
          };
            ...
            if (sessionData.step === "service") {
              ...
              if (normalizedMessage.match(/^\d{2}-\d{2}$/)) {
                ...
                if (
                  ...
                  return "Data inválida. Informe uma data válida no formato dia-mês (ex: 13-02).";
                }
                ...
                if (hours.dayClosed) {
                  ...
                  return `Data selecionada: ${fullDateStr}. Horários disponíveis:\n${hours.availableTimes.join("\n")}\n\nInforme a hora (ex: 14:00).`;
                }
              } else {
              ...
              if (normalizedMessage.match(/^\d{2}:\d{2}$/)) {
                ...
                await whatsappService.sendText(`${adminPhone}@s.whatsapp.net`, adminMessage);
